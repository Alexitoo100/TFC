...
function guidv4($data = null) {
    ...
    return vsprintf('%s%s-%s-%s-%s-%s%s%s', str_split(bin2hex($data), 4));
        ...
        $passwordRepeticion = $_POST['passwordRepeticion'];
        ...
        $pass="tfg";
            ...
            if ($username === '' || $nombre === '' || $apellidos === '' || $correo === '' || $password === '' || $passwordRepeticion === '') {
                echo "empty";
            } else if (!(filter_var($correo, FILTER_VALIDATE_EMAIL))) {
                echo "correo-invalido";
            } else if ($password != $passwordRepeticion) {
                echo "password-no-coincide";
            } else if (strlen($password) < 8 && strlen($password) != "") { 
                echo "password-muy-corta";
            } else {
                if ($tuplas == '') {
                    // Preparamos la inserción del usuario.
